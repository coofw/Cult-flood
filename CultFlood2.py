...
# ========== КОНФИГ ==========
BOT_TOKEN = "8573524597:AAGlFY6szL1HD2RmU4EHvDeSty9oKQwcKHU"  # ЗАМЕНИ НА РЕАЛЬНЫЙ!
...
class DatabaseManager:
    ...
    def close_ticket(self, ticket_id):
        """Закрыть тикет (удалить из активных, добавить в архив)"""
    ...
    def restore_ticket_from_archive(self, archive_id):
        """Восстановить тикет из архива в активные"""
...
@dp.callback_query(F.data.startswith("ticket_") and not F.data.startswith("ticket_reply_") and not F.data.startswith("ticket_close_"))
...
async def admin_archive(message: Message):
    ...
    for item in archive_items:
        ...
        builder.button(text=f"#{original_id}", callback_data=f"archive_{archive_id}")
...
@dp.callback_query(F.data.startswith("archive_") and not F.data.startswith("archive_restore_"))
