...
class DatabaseManager:
    ...
    def cleanup_archive(self, days=7):
        ...
        self.conn.commit()
        if deleted:
            logger.info(f"🧹 Удалено {deleted} старых записей из архива")
        return deleted
...
async def send_ticket_message(message: Message, state: FSMContext):
    ...
    # ОПОВЕЩЕНИЕ АДМИНОВ О НОВОМ ТИКЕТЕ
...
async def process_application(message: Message, state: FSMContext):
    ...
    if len(msg_text) > 4000:
        ...
        await message.answer("⚠️ Ваше сообщение слишком длинное, оно было обрезано до 4000 символов.")
    ...
    # ОПОВЕЩЕНИЕ АДМИНОВ О НОВОЙ АНКЕТЕ
    admins = db.get_all_admin_ids()
    for admin_id in admins:
        if db.get_admin_level(admin_id) >= 1:
            try:
                ...
                logger.info(f"✅ Уведомление об анкете #{app_id} отправлено админу {admin_id}")
            except Exception as e:
                logger.error(f"❌ Ошибка отправки уведомления админу {admin_id}: {e}")
...
async def process_user_reply(message: Message, state: FSMContext):
    ...
    # Оповещаем админов о новом сообщении
    admins = db.get_all_admin_ids()
    for admin_id in admins:
        if db.get_admin_level(admin_id) >= 2:
            try:
