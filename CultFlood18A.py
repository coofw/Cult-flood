...
def parse_time_input(time_str: str) -> Tuple[Optional[timedelta], str]:
    """Парсит ввод времени: 15m, 2h, 5d, 3M, 1y, 0/never"""
...
def format_expires_text(expires_at: Optional[str]) -> str:
    """Форматирует срок для отображения (ФИКС ВИЗУАЛЬНОГО БАГА)"""
    if not expires_at:
        return "бессрочно"
    try:
...
async def list_admins(call: CallbackQuery):
    ...
    for admin_id, level, added_at, expires_at in admins:
        ...
        # ФИКС: правильное отображение срока
        if expires_at:
            try:
                expire_date = datetime.fromisoformat(expires_at)
                if expire_date < datetime.now():
                    ...
                    if days > 0:
                        if days >= 365:
                            years = days // 365
                            expires_text = f"{years} г"
                        elif days >= 30:
                            months = days // 30
                            expires_text = f"{months} мес"
                        else:
                            expires_text = f"{days} дн"
                    elif hours > 0:
                        expires_text = f"{hours} ч"
                    elif minutes > 0:
                        expires_text = f"{minutes} мин"
                    else:
...
async def check_admin_db(message: Message):
    ...
    for user_id, level, expires_at in rows:
        username = db.get_username(user_id)
        text += f"🆔 {user_id} (@{username}) | уровень {level}\n"
        if expires_at:
            ...
            try:
                ...
                if expire_date > now:
                    delta = expire_date - now
                    text += f"   ⏱️ осталось: {delta}\n"
                else:
                    text += f"   ⏱️ ИСТЁК\n"
            except:
                text += f"   ⏱️ ОШИБКА ПАРСИНГА\n"
        else:
            text += f"   📅 бессрочно (NULL)\n"
        text += "\n"
