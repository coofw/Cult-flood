...
# ========== ПАРСИНГ ВРЕМЕНИ (ЖЁСТКАЯ ВЕРСИЯ) ==========
def parse_time_input(time_str: str) -> Tuple[Optional[timedelta], str, Optional[str]]:
    """
    Парсит ввод времени. Возвращает (timedelta, текст_для_отображения, iso_дата_для_лога)
    """
    time_str = time_str.strip().lower()
    
    # Бессрочно
    if time_str == '0' or time_str == 'never':
        return None, "бессрочно", None
    ...
    if match:
        ...
        return delta, f"{value} мин", expire_date.isoformat()
    ...
    if match:
        ...
        expire_date = datetime.now() + delta
        return delta, f"{value} ч", expire_date.isoformat()
    ...
    if match:
        ...
        expire_date = datetime.now() + delta
        return delta, f"{value} дн", expire_date.isoformat()
    
    # Месяцы: 3M (заглавная M)
    match = re.match(r'^(\d+)M$', time_str)
    if match:
        value = int(match.group(1))
        delta = timedelta(days=value * 30)
        expire_date = datetime.now() + delta
        return delta, f"{value} мес", expire_date.isoformat()
    ...
    if match:
        ...
        expire_date = datetime.now() + delta
        return delta, f"{value} г", expire_date.isoformat()
...
class DatabaseManager:
    ...
    def add_admin(self, user_id, level, added_by, delta: Optional[timedelta] = None):
        ...
        if delta:
            ...
            logger.info(f"✅ ДОБАВЛЕН АДМИН: user_id={user_id}, level={level}, expires_at={expires_at}")
        else:
            logger.info(f"✅ ДОБАВЛЕН АДМИН (бессрочно): user_id={user_id}, level={level}")
...
async def add_admin_time(message: Message, state: FSMContext):
    ...
    delta, delta_text, _ = parse_time_input(time_str)
...
@dp.message(Command("check_admin_db"))
async def check_admin_db(message: Message):
    if message.from_user.id not in MASTER_ADMIN_IDS:
        return
    
    db.cursor.execute("SELECT user_id, level, expires_at FROM admins")
    rows = db.cursor.fetchall()
    
    if not rows:
        await message.answer("В БД нет админов")
        return
    
    text = "📊 **Содержимое таблицы admins:**\n\n"
    for user_id, level, expires_at in rows:
        username = db.get_username(user_id)
        expires_text = format_expires_text(expires_at)
        text += f"🆔 {user_id} (@{username}) | уровень {level} | срок: {expires_text}\n"
        if expires_at:
            text += f"   📅 raw: {expires_at}\n"
