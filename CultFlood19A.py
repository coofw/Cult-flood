...
class DatabaseManager:
    ...
    def setup(self):
        ...
        # ФИКС: applications — храним оригинальный текст
    ...
    def cleanup_expired_admins(self):
        """Удаляет истёкших админов (раз в 24 часа)"""
        now = datetime.now().isoformat()
        self.cursor.execute("DELETE FROM admins WHERE expires_at IS NOT NULL AND expires_at < ?", (now,))
        deleted = self.cursor.rowcount
        self.conn.commit()
        if deleted:
            logger.info(f"🧹 Удалено {deleted} истёкших админов")
        return deleted
    
    # ========== КОДЫ ==========
    def generate_admin_code(self, code, level, delta, max_uses, created_by):
        expires_at = (datetime.now() + delta).isoformat()
        self.cursor.execute("INSERT OR REPLACE INTO admin_codes (code, level, expires_at, max_uses, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def use_admin_code(self, code, user_id):
        ...
        if current_level == 0:
            self.add_admin(user_id, code_level, 0, None)
    ...
    # ========== АНКЕТЫ ==========
    def create_application(self, user_id, username, original_text):
        self.cursor.execute("INSERT INTO applications (user_id, username, original_text, created_at) VALUES (?, ?, ?, ?)",
                           (user_id, username, original_text, datetime.now().isoformat()))
        self.conn.commit()
        return self.cursor.lastrowid
    
    def get_pending_applications(self):
        self.cursor.execute("SELECT id, user_id, username, original_text, created_at FROM applications WHERE status = 'pending' ORDER BY created_at ASC")
        return self.cursor.fetchall()
    
    def get_application(self, app_id):
        self.cursor.execute("SELECT id, user_id, username, original_text, status, created_at FROM applications WHERE id = ?", (app_id,))
    ...
    # ========== АРХИВ ==========
    def add_to_archive(self, item_type, original_id, user_id, username, data):
    ...
    def cleanup_archive(self, days=7):
        """Удаляет записи из архива старше N дней"""
        cutoff = (datetime.now() - timedelta(days=days)).isoformat()
        self.cursor.execute("DELETE FROM archive WHERE closed_at < ?", (cutoff,))
        deleted = self.cursor.rowcount
        self.conn.commit()
        if deleted:
            logger.info(f"🧹 Удалено {deleted} старых записей из архива (старше {days} дней)")
        return deleted
    
    # ========== ОСТАЛЬНЫЕ МЕТОДЫ (ТИКЕТЫ, ПОЛЬЗОВАТЕЛИ) ==========
    ...
    def close_ticket(self, ticket_id):
        ...
        self.add_to_archive('ticket', ticket_id, user_id, username, archive_data)
    ...
    def restore_ticket_from_archive(self, archive_id):
        item = self.get_archive_item(archive_id)
        if not item or item[1] != 'ticket':
            return False
...
# ========== ОТПРАВИТЬ АНКЕТУ (ФИКС: ПРОСТО ПЕРЕСЫЛАЕМ) ==========
...
async def send_application_form(message: Message, state: FSMContext):
    ...
    await message.answer(form, parse_mode="Markdown")
...
async def process_application(message: Message, state: FSMContext):
    ...
    # Сохраняем в БД оригинальный текст
    app_id = db.create_application(user_id, username, msg_text)
    ...
    # Отправляем админу: пересылаем оригинальное сообщение + служебное
    for admin_id in ADMIN_IDS:
        try:
            ...
            # Отправляем служебное сообщение с информацией
            await bot.send_message(
                admin_id,
            ...
            logger.error(f"Ошибка отправки анкеты админу {admin_id}: {e}")
...
async def admin_applications(message: Message):
    ...
    for app in apps:
        app_id, user_id, username, original_text, created_at = app
...
async def view_application(call: CallbackQuery):
    ...
    user_id, username, original_text, status, created_at = app[1], app[2], app[3], app[4], app[5]
...
async def add_admin_time(message: Message, state: FSMContext):
    ...
    if delta is None and time_str not in ['0', 'never']:
        await message.answer("❌ Неверный формат. Примеры: 15m, 2h, 5d, 3M, 1y, 0, never")
        return
...
async def create_code_time(message: Message, state: FSMContext):
    ...
    if delta is None:
        await message.answer("❌ Неверный формат. Примеры: 15m, 2h, 5d, 3M, 1y")
        return
...
async def create_code_max_uses(message: Message, state: FSMContext):
    try:
        ...
        db.generate_admin_code(code, level, delta, max_uses, message.from_user.id)
...
async def cleanup_scheduler():
    """Раз в 24 часа чистим истёкших админов и старый архив"""
    while True:
        await asyncio.sleep(24 * 3600)  # Раз в сутки
        try:
            deleted_admins = db.cleanup_expired_admins()
            deleted_archive = db.cleanup_archive(days=7)
            logger.info(f"🧹 Очистка: админов удалено {deleted_admins}, архива {deleted_archive}")
        except Exception as e:
