...
class DatabaseManager:
    ...
    def add_admin(self, user_id, level, added_by, expires_days=None):
        expires_at = None
    ...
    def upgrade_admin_level(self, user_id, new_level):
        """Повышает уровень админа, если новый уровень выше текущего"""
    ...
    # ========== КОДЫ ДЛЯ АДМИНКИ ==========
    def generate_admin_code(self, level, expires_days, max_uses, created_by) -> str:
    ...
    def use_admin_code(self, code, user_id) -> tuple:
        ...
        # Если пользователь не админ — добавляем
        if current_level == 0:
            self.add_admin(user_id, code_level, 0, expires_days=None)
        ...
        # Если пользователь уже админ — повышаем уровень
...
class RemoveFromBlacklistState(StatesGroup):
    waiting_for_user_id = State()
...
async def send_ticket_message(message: Message, state: FSMContext):
    ...
    # Уведомляем всех админов с уровнем >=2
...
async def process_application(message: Message, state: FSMContext):
    ...
    # Уведомляем всех админов с уровнем >=1
    admins = db.get_all_admins()
    for admin_id, level, _, _ in admins:
        if level >= 1:
            try:
...
async def process_user_reply(message: Message, state: FSMContext):
    ...
    # Уведомляем админов с уровнем >=2
    admins = db.get_all_admins()
    for admin_id, level, _, _ in admins:
        if level >= 2:
            try:
...
async def admin_add_blacklist_start(message: Message, state: FSMContext):
    if db.get_admin_level(message.from_user.id) < 3:
        return
...
async def admin_remove_blacklist_start(message: Message, state: FSMContext):
    if db.get_admin_level(message.from_user.id) < 3:
        return
    
    await state.set_state(RemoveFromBlacklistState.waiting_for_user_id)
    await message.answer(
...
@dp.message(RemoveFromBlacklistState.waiting_for_user_id)
async def admin_remove_blacklist_user_id(message: Message, state: FSMContext):
    try:
        user_id = int(message.text.strip())
        
        if not db.is_banned(user_id):
            await message.answer(f"❌ Пользователь {user_id} не находится в чёрном списке!")
            await state.clear()
            return
        ...
        await message.answer(
            f"✅ **Пользователь удалён из ЧС!**\n\n"
            f"🆔 ID: {user_id}\n"
            f"👤 Username: @{username}\n\n"
            f"Пользователь получил уведомление о разблокировке."
        )
...
async def add_admin_level(message: Message, state: FSMContext):
    try:
        ...
        await state.set_state(AddAdminState.waiting_for_days)
        await message.answer(
            "✏️ Введите срок действия в днях (0 = бессрочно):\n\n"
            "Пример: 30 (месяц), 0 (навсегда)\n\n"
            "🔙 Отмена — /cancel"
        )
    except ValueError:
        await message.answer("❌ Введите число (1, 2 или 3)")

@dp.message(AddAdminState.waiting_for_days)
async def add_admin_days(message: Message, state: FSMContext):
    try:
        ...
        db.add_admin(user_id, level, message.from_user.id, days if days > 0 else None)
        ...
        await message.answer("❌ Введите число (0 или количество дней)")
...
async def list_admins(call: CallbackQuery):
    ...
    for admin_id, level, added_at, expires_at in admins:
        ...
        expires_text = f"до {expires_at[:16]}" if expires_at else "бессрочно"
...
async def create_code_level(message: Message, state: FSMContext):
    try:
        ...
        await state.set_state(CreateCodeState.waiting_for_days)
        await message.answer(
            "✏️ Введите срок действия кода в днях:\n\n"
            "Пример: 30 (код будет активен месяц)\n\n"
            "🔙 Отмена — /cancel"
        )
    except ValueError:
        await message.answer("❌ Введите число (1, 2 или 3)")

@dp.message(CreateCodeState.waiting_for_days)
async def create_code_days(message: Message, state: FSMContext):
    try:
        days = int(message.text.strip())
        if days <= 0:
            await message.answer("❌ Срок должен быть больше 0 дней")
            return
        await state.update_data(code_days=days)
...
async def create_code_max_uses(message: Message, state: FSMContext):
    try:
        ...
        code = db.generate_admin_code(level, days, max_uses, message.from_user.id)
...
async def list_codes(call: CallbackQuery):
    ...
    for code, level, expires_at, max_uses, used, created_at in codes:
        ...
        expires_short = expires_at[:16] if expires_at else "—"
        text += f"🔹 `{code}`\n   📊 {level_name} | 📅 до {expires_short}\n   🔢 {used}/{max_uses}\n\n"
...
async def back_to_blacklist(call: CallbackQuery):
    await admin_blacklist_view(call.message)
