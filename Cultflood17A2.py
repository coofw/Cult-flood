...
class DatabaseManager:
    ...
    def add_admin(self, user_id, level, added_by, expires_days=None):
        ...
        if expires_days is not None and expires_days > 0:
            ...
            logger.info(f"Добавлен админ {user_id} на {expires_days} дней (до {expires_at})")
        else:
    ...
    def add_to_blacklist(self, user_id, reason):
        ...
        self.conn.commit()
        logger.info(f"Пользователь {user_id} добавлен в ЧС по причине: {reason}")
...
async def list_admins(call: CallbackQuery):
    ...
    for admin_id, level, added_at, expires_at in admins:
        ...
        expires_text = db.get_admin_expires_text(expires_at)
