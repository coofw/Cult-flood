...
class DatabaseManager:
    ...
    def add_admin(self, user_id, level, added_by, expires_days=None):
        """Добавляет админа. expires_days = None или 0 → бессрочно, число → на N дней"""
        expires_at = None
    ...
    def get_admin_expires_text(self, expires_at):
        """Возвращает читаемый текст срока действия"""
        if not expires_at:
            return "бессрочно"
        try:
...
def get_blacklist_actions_keyboard():
    builder = InlineKeyboardBuilder()
...
def get_blacklist_remove_keyboard(user_id):
    builder = InlineKeyboardBuilder()
    builder.button(text="❌ Разбанить", callback_data=f"blacklist_remove_{user_id}")
    builder.button(text="🔙 Назад", callback_data="blacklist_back")
    builder.adjust(2)
    return builder.as_markup()
...
@dp.callback_query(F.data == "blacklist_back")
async def blacklist_back(call: CallbackQuery):
...
async def add_admin_level(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            "Пример: 1 (на 1 день), 30 (на месяц), 0 (навсегда)\n\n"
            "🔙 Отмена — /cancel"
        )
    except ValueError:
...
async def add_admin_days(message: Message, state: FSMContext):
    try:
        ...
        # ФИКС: если days == 0 → бессрочно, иначе на days дней
            ...
            try:
                ...
                if expires_date < datetime.now():
                    ...
                    if days_left == 0:
                        ...
                        weeks = days_left // 7
                        expires_text = f"{weeks} недель"
                    else:
                        months = days_left // 30
                        expires_text = f"{months} месяцев"
            except:
                expires_text = "ошибка"
        else:
            expires_text = "бессрочно"
